...
#' For more information, see \href{http://hua-zhou.github.io/media/pdf/GainesKimZhou08CLasso.pdf}{Gaines et al. (2018)}.
#'
...
#'
#' @section Details:
#' The Constrained Lasso as in Gaines et al. (2018) minimizes
...
constrLassoReg <- function(X, y, lambda, Aeq=NULL, beq=NULL, A=NULL, b=NULL, penidx=NULL, method="QP"){
		...
		if(abs(lambda) < 1e-16){
		 ...
		 # with penalization
			if(method=="CD"){
				...
				glmnet_res <- glmnet(Xwt, ywt, alpha=1, lambda=lambda, penalty.factor=penidx, maxit=1000, intercept=FALSE, standardize=TRUE)
				...
				betahat <- matrix(opt_sol[1:p] - opt_sol[(p+1):length(opt_sol)], p, 1)
				dualEq <-  rep(0,0)
				dualIneq <-  rep(0,0)
			...
			# optimizer
			x <- OP(Q_objective(H, L=t(f)), L_constraint(L=Amatrix, dir=c(rep("==", m1), rep("<=", m2)), rhs=bvector))
			opt <- ROI_solve(x, solver="qpoases")
			opt_sol <-  opt$message$primal_solution
	...
	return(list("betahat"=betahat, "dualEq"=dualEq, "dualIneq"=dualIneq))
}
...
constrLassoPath <- function(X, y, Aeq=NULL, beq=NULL, A=NULL, b=NULL, penidx=NULL, init_method="QP",  epsilon=1e-4, stop_lambdatol=1e-7, ceiling_tol = 1e-10, zeros_tol = 1e-20, verbose=FALSE){
	...
	if(n < p){
		...
		if (epsilon <= 0) {
			warning("The ridge tuning parameter epsilon must be positive, switching to default value (1e-4).")
			epsilon <- 1e-4
		}
		...
		R <- qr(X)$qr[1:p,]
		rankX <- sum(matrix(abs(diag(R))) > abs(R[1])*max(n,p)*.Machine$double.eps)
		rankX <- qr(X)$rank
	...
	if(init_method=="LP"){
		...
		lpsol <- lp(direction="min", obj_fun, constr_mat, constr_dir, constr_rhs, compute.sens=TRUE)
		...
		# initialize sets
		dualpathIneq[which(dualpathIneq[,1]<0),1] <- 0 #fix negative duals
		...
		subgrad <- t(X)%*%resid - t(Aeq)%*%dualpathEq[,1] - t(A)%*%dualpathIneq[,1]
	...
	objValPath[,1] <- 0.5*(sum((y- X%*%betaPath[,1])^2)) + lambdaPath[,1]*sum(abs(betaPath[,1]))
	...
	for(k in 2:maxiters){
		...
		M <- cbind(H[setActive, setActive], t(matrix(Aeq[,setActive], ncol=nActive)), t(matrix(A[setIneqBorder, setActive], ncol=nActive)))

		M <- rbind(M, cbind(rbind(matrix(Aeq[,setActive], ncol=nActive), matrix(A[setIneqBorder, setActive], ncol=nActive)), matrix(0, m1+nIneqBorder, m1+nIneqBorder)))

		## calculate derivative
		# try using a regular inverse first, otherwise Moore-Penrose Inverse
		error.hand.inv <- function(e){
			dir <- -(ginv(M)%*%rbind(matrix(subgrad[setActive], nActive, 1), matrix(0, m1+nIneqBorder,1)))
			printer("Moore-Penrose-Inverse used.")
			return(dir)
		}
		dir <- tryCatch(dirsgn*(solve(M, rbind(matrix(subgrad[setActive], nActive, 1), matrix(0, m1+nIneqBorder,1)))), error=error.hand.inv)

		if(nInActive!=0){
			dirSubgrad <- - cbind(matrix(H[!setActive,setActive], ncol=nActive), t(matrix(Aeq[,!setActive], ncol=nInActive)), t(matrix(A[setIneqBorder, !setActive], ncol=nInActive))) %*%dir
		}else{
		...
		inactSlowNegIdx <- which((1*dirsgn - ceiling_tol) <= subgrad[!setActive] & subgrad[!setActive] <= (1*dirsgn + ceiling_tol) &	1*dirsgn < dirSubgrad)
		# Positive subgradient
		inactSlowPosIdx <- which((-1*dirsgn - ceiling_tol) <= subgrad[!setActive] & subgrad[!setActive] <= (-1*dirsgn + ceiling_tol) & dirSubgrad < -1*dirsgn)

		## "Active" coeficients estimated as 0 with potential sign mismatch
		# Positive subgradient but negative derivative
		signMismatchPosIdx = which((0 - ceiling_tol) <= subgrad[setActive] & subgrad[setActive] <= (1 + ceiling_tol) & dirsgn*dir[1:nActive] <= (0 - ceiling_tol)  & betaPath[setActive, k-1] == 0)
		# Negative subgradient but positive derivative
		signMismatchNegIdx <- which((-1 - ceiling_tol) <= subgrad[setActive] & subgrad[setActive] <= (0 + ceiling_tol) & (0 + ceiling_tol) <= dirsgn*dir[1:nActive] & betaPath[setActive, k-1] == 0)
		...
		while(length(inactSlowNegIdx)!=0 || length(inactSlowPosIdx)!=0  || length(signMismatchPosIdx)!=0  || length(signMismatchNegIdx)!=0){
			...
			while(length(inactSlowNegIdx)!=0){
				...
				inactiveCoeffs <- which(!setActive) # indices corresponding to inactive coefficients
				viol_coeff <- inactiveCoeffs[inactSlowNegIdx] # identify problem coefficient
				setActive[viol_coeff] <- TRUE # put problem coefficient back into active set
				nActive <- length(which(setActive)) # determine new number of active/inactive coefficients
				nInActive <- length(which(!setActive))
				nIneqBorder <- length(which(setIneqBorder)) # determine number of active/binding inequality constraints
			...
			while(length(inactSlowPosIdx)!=0){
				...
				inactiveCoeffs <- which(!setActive) # indices corresponding to inactive coefficients
				viol_coeff <- inactiveCoeffs[inactSlowPosIdx] # identify problem coefficient
				setActive[viol_coeff] <- TRUE # put problem coefficient back into active set;
				nActive <- length(which(setActive)) # determine new number of active/inactive coefficients
				nInActive <- length(which(!setActive))
				nIneqBorder <- length(which(setIneqBorder)) # determine number of active/binding inequality constraints
			...
			while(length(signMismatchPosIdx)!=0){
				...
				activeCoeffs <- which(setActive) # indices corresponding to inactive coefficients
				viol_coeff <- activeCoeffs[signMismatchPosIdx] # identify problem coefficient
				setActive[viol_coeff] <- FALSE # put problem coefficient back into active set;
				nActive <- length(which(setActive)) # determine new number of active/inactive coefficients
				nInActive <- length(which(!setActive))
				nIneqBorder <- length(which(setIneqBorder)) # determine number of active/binding inequality constraints
			...
			while(length(signMismatchNegIdx)!=0){
				...
				activeCoeffs <- which(setActive) # indices corresponding to inactive coefficients
				viol_coeff <- activeCoeffs[signMismatchNegIdx] # identify problem coefficient
				setActive[viol_coeff] <- FALSE # put problem coefficient back into active set
				nActive <- length(which(setActive)) # determine new number of active/inactive coefficients
				nInActive <- length(which(!setActive))
				nIneqBorder <- length(which(setIneqBorder)) # determine number of active/binding inequality constraints
		...
		dirResidIneq <- matrix(A[!setIneqBorder, setActive], nrow=length(which(!setIneqBorder)), ncol=nActive)%*%dir[1:nActive]
		...
		t1 <- dirsgn*lambdaPath[,k-1]*(1 - subgrad[!setActive])/(dirSubgrad - 1)
		...
		nextlambdaIneq[!setIneqBorder,] <- as.matrix(-dirsgn*residIneq[!setIneqBorder], length(which(!setIneqBorder)), 1)/(as.matrix(dirResidIneq, length(which(!setIneqBorder)), 1))

		# Active inequality constraint becoming deactive #
		nextlambdaIneq[setIneqBorder,] <- as.matrix(-dirsgn*dualpathIneq[setIneqBorder, k-1])/as.matrix(dir[-(1:(nActive+m1))], nIneqBorder,1)

		# ignore delta lambdas equal to zero
		nextlambdaIneq[nextlambdaIneq <= ceiling_tol,] <- Inf

		# find smallest lambda
		chglambda <- min(rbind(nextlambdaBeta, nextlambdaIneq), na.rm=TRUE)

		# find all indices corresponding to this chglambda
		idx <- which((rbind(nextlambdaBeta, nextlambdaIneq) - chglambda) <= ceiling_tol)
		...
		betaPath[setActive, k] <-  betaPath[setActive, k-1] + dirsgn*chglambda*dir[1:nActive]
		...
		subgrad[!setActive] <- as.matrix(lambdaPath[,k-1]*subgrad[!setActive,] + dirsgn*chglambda*dirSubgrad)/lambdaPath[,k]

		## Update dual variables

		# update duals1 (lagrange multipliers for equality constraints)
		dualpathEq[, k] <- dualpathEq[,k-1] + dirsgn*chglambda*as.matrix(dir[(nActive+1):(nActive+m1)], m1, 1)

		# update duals2 (lagrange multipliers for inequality constraints)
		dualpathIneq[setIneqBorder, k] <-  dualpathIneq[setIneqBorder, k-1] + dirsgn*chglambda*as.matrix(dir[(nActive+m1+1):nrow(dir)], nIneqBorder, 1)
		...
		objValPath[k] = 0.5*(sum((y- X%*%betaPath[,k])^2)) + lambdaPath[k]*sum(abs(betaPath[,k]))
	...
	return(list("betaPath"=betaPath, "lambdaPath"=lambdaPath, "objValPath"=objValPath, "dfPath"= dfPath))
}
